---
title: 비동기 처리를 다루는 두 가지 방식 - 스레드와 이벤트
lang: ko
layout: post
---
...
이번 글에서는 비동기 프로그래밍의 두 가지 접근 방식인 스레드 기반 모델과 이벤트 루프 기반 모델을 비교하고, 각각의 특징과 한계를 살펴보았다. 스레드 기반 모델은 CPU 바운드 작업에서 강점을 가지지만, 블로킹 I/O가 많은 환경에서는 스레드 폭발과 컨텍스트 스위칭 비용이라는 문제가 발생할 수 있다. 반면 이벤트 루프 기반 모델은 I/O 중심의 동시 처리에 유리하지만, CPU-heavy 작업에서는 전체 처리 지연을 초래할 수 있다.

Spring MVC와 FastAPI 시나리오를 통해 문제 상황부터 간단한 해결까지도 살펴보았다. Spring MVC에서는 Thread-per-Request 구조로 인해 스레드 풀을 초과하는 요청이 들어오면 대기 큐가 포화되고 응답 지연이 발생할 수 있었다. 이를 해결하기 위해 스레드 풀 크기를 늘리거나, CompletableFuture와 WebClient를 활용해 블로킹 I/O를 비동기 처리함으로써 워커 스레드의 효율성을 높일 수 있다. 반면 FastAPI에서는 이벤트 루프 기반 비동기로 I/O 작업을 논블로킹 처리하고, CPU-heavy 작업을 별도 워커 스레드 풀로 위임함으로써 단일 스레드에서도 다수 요청을 효율적으로 처리할 수 있다.

더 나아가 코루틴과 가상 스레드를 살펴봄으로써, 비동기 처리를 언어 차원에서 달성하기 위한 노력을 확인할 수 있었다. 코루틴은 함수 단위에서 제어를 일시 중단하고 재개할 수 있도록 설계되어, 이벤트 루프 기반 논블로킹 I/O 환경에서 경량 작업 단위를 제공한다. 반면, 가상 스레드는 OS 스레드(플랫폼 스레드)를 기반으로 JVM이 관리하며 실행되므로, 기존 동기 코드를 거의 그대로 사용하면서도 수만 개의 동시 작업을 효율적으로 처리할 수 있도록 확장성을 제공한다.

코루틴과 가상 스레드에 대해서는 간략하게만 살펴봤기 때문에, 자세한 구현 방식을 추후 따로 다루어도 좋겠다.
