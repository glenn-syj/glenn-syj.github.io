...
기호의 세계는 이산적이다. 서로 다른 두 단어는 기호로서 독립적이다. 서로 다른 두 벡터는 특성을 축으로 나열되어 있다. 사과와 배는 기호로서는 다르지만, 벡터로서는 여러 특성값을 공유할 수 있다. 즉, 벡터는 비교 가능하다.

그러나 이 특성은 먼저 정의되어 존재하는 것이 아니라, 결과에 맞춰 수치를 깎아 나가는 과정에서 다듬어진다. 초기에는 임베딩 행렬 안의 벡터가 무의미한 난수 배열로 존재한다. 바로 임베딩 테이블이다.

여기에서 "거리"가 중요해진다. 거리는 상태를 나타내는 것이기도 하지만, 동시에 가까워지고 멀어지는 운동이라는 가능성을 포함하는 개념이다. 이 거리는 역전파(backpropagation)을 통해 줄어든다.

모델은 사전 학습을 통해 벡터와 벡터 사이 연산 결과가 사전에 마련해둔 정답 인덱스에 가까워지도록 벡터가 수정된다. 이 연산은 선형 변환과 비선형 활성화 함수의 반복이다.

따라서 벡터가 가지는 특성값은 정답에 가까이 가기 위해 소멸시키고 증폭시키는 필터링을 거친 결과다. 특성값은 사전에 정의한 의미가 아니라 정답 인덱스를 향한 연산 결과다.

그렇다면 왜 벡터는 '사과'와 '배' 같이 유사성을 반영한다고 여겨질까? 이는 통과하는 층(Weight Matrices)이 같기 때문이다. 동일한 행렬층과 곱해지면서 같은 정답 인덱스의 확률을 높이는 방향으로 손실이 최소화된다. 따라서 역전파 과정에서 두 벡터의 성분값이 유사해진다.
