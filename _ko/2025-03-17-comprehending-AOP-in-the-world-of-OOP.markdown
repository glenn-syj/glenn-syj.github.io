...
앨런 케이가 말하는 OOP가 시스템적 측면을 강조하고 있음은 우연이 아닙니다. Smalltalk 혹은 앨런 케이의 OOP를 저는 현대 OOP의 메시지 중심 객체지향의 뿌리라고 일컫고 싶습니다. 특히 Smalltalk 커뮤니티의 켄트 백(Kent Beck)과 워드 커닝햄(Ward Cunningham)이 CRC(Class-Responsibility-Collaboration) 기법을 통해 객체의 자율성과 책임에 기반한 객체 간의 협력 네트워크를 형성하는 일을 제시한 것은 우연이 아니라고 생각합니다. 또한, 설계적 측면에서 시스템 행위를 동적으로 수정하고 이벤트 기반의 아키텍처를 형성할 수 있는 기반을 닦아놓은 것 역시 Smalltalk의 영향입니다.

#### (2) Simula: 모델링 중심 객체지향

Simula는 모델링 중심 객체 지향의 뿌리입니다. Simula는 실세계를 소프트웨어로 모델링하는 구조적 메커니즘에 초점을 맞추는 언어입니다. 따라서 복잡한 시스템을 모델링하기 위해 클래스와 객체라는 개념을 도입했으며, 이는 현대 객체지향 언어의 기본 구조가 되었습니다. Smalltalk 역시 객체 개념을 Simula에서 영향 받았음을 밝힌 적 있습니다.

Simula의 가장 중요한 기여는 클래스 기반 상속 체계와 타입 시스템을 통한 모델링 능력입니다. Simula는 실세계 엔티티를 클래스로 추상화하고, 이를 상속 계층으로 표현하는 방법을 제시했습니다.

Simula의 모델링 중심 접근법은 다음과 같은 특징을 가집니다:

1. **클래스와 객체의 분리**: 클래스를 객체의 청사진으로 정의하고, 객체를 클래스의 인스턴스로 생성하는 개념
2. **상속 계층**: 일반화/특수화 관계를 표현하는 클래스 계층 구조
3. **가상 메서드**: 다형성을 지원하는 초기 형태의 메서드 오버라이딩
4. **정적 타입 시스템**: 컴파일 시간에 타입 안전성을 보장하는 메커니즘

이러한 특성들은 소프트웨어 엔지니어링에서 대규모 시스템을 설계하고 구조화하는 데 강력한 도구를 제공했습니다. Simula의 접근 방식은 특히 도메인 모델링과 분석 단계에서 실세계 개념을 소프트웨어 구조로 매핑하는 데 도움이 되었고, UML(Unified Modeling Language)과 같은 객체지향 모델링 언어가 역시 같은 영향 하에 있습니다. 실제로 UML의 창시자 중 한 명인 Grady Booch는 Simula의 모델링 능력에서 영감을 받았다고 여러 차례 언급했습니다.

#### (3) 두 뿌리의 융합과 발전

현대 객체지향 프로그래밍은 Smalltalk의 메시지 중심 접근법과 Simula의 모델링 중심 접근법이 다양한 방식으로 융합되고 발전한 결과입니다. 이 두 가지 접근법은 상호보완적이며, 각각 객체지향 패러다임의 다른 측면을 강조합니다.
...
객체지향 설계 방법론 역시 이 두 가지 뿌리의 영향을 받았습니다. 도메인 주도 설계(Domain-Driven Design)는 Simula의 모델링 중심 접근법에서 영감을 받아 비즈니스 도메인을 소프트웨어 모델로 표현하는 데 초점을 맞춥니다. 반면, 책임 주도 설계(Responsibility-Driven Design)는 Smalltalk의 메시지 중심 접근법에서 영향을 받아 객체의 책임과 협력에 초점을 맞춥니다.

이러한 두 가지 접근법의 융합은 객체지향 프로그래밍의 풍부함과 다양성을 가져왔습니다. 그러나 이 융합 과정에서 각 접근법의 일부 핵심 통찰이 희석되거나 잊혀지기도 했습니다. 특히 Smalltalk의 순수한 메시지 전송 모델과 동적 시스템 개념은 주류 객체지향 언어에서 완전히 구현되지 않았으며, 이는 현대 객체지향 시스템이 직면한 일부 한계와 관련이 있습니다.
