...
Simula, Smalltalk 등 초기 객체지향 언어에서 객체는:

- 상태(state)
- 동작(method)
- 메시지 수신(message passing)
  을 갖는 **프로그램 안의 구조화된 단위**였습니다.

이 시기에 객체는 애플리케이션 내부에서 하나의 "엔티티"로, 주로 메모리 내에 존재하는 클래스의 인스턴스를 의미했습니다.
...
객체 개념의 이러한 확장은 객체지향이 단순한 프로그래밍 패러다임을 넘어 시스템 설계 전반의 사고방식이 되었음을 의미합니다. 이는 앨런 케이가 처음 객체지향을 구상했을 때 가졌던 비전—"모든 것은 객체이며, 객체들은 메시지를 주고받는다"—가 프로그래밍 언어의 경계를 넘어 분산 시스템 설계의 원리로 확장되었음을 보여줍니다.

이러한 관점에서 AOP(관점 지향 프로그래밍)는 이렇게 확장된 객체 세계에서 횡단 관심사(cross-cutting concerns)를 다루기 위한 자연스러운 진화로 볼 수 있습니다. 객체의 범위가 확장됨에 따라, 로깅, 보안, 트랜잭션 관리와 같은 공통 관심사를 처리하는 방식도 함께 진화해야 했기 때문입니다.

결론적으로, 객체 개념의 확장은 OOP의 실용적 적용이 어떻게 진화해왔는지를 보여주는 중요한 관점입니다. 이는 "진짜 OOP"를 찾는 이론적 논쟁보다, 객체 개념이 어떻게 다양한 문제 영역과 시스템 수준에 적용되어 왔는지를 이해하는 것이 더 중요함을 시사합니다.
